We will start with a word on notation within this and the following appendix. To
start with, it is important to note which numbers are real, and may be
represented by a real number data storage format of suitable precision, and
which numbers are complex. Complex numbers do not necessarily need to be
represented by a general complex number data storage, especially if symmetry
properties constrain the possible phase arguments of the complex number. To that
end, to make such distinctions more transparent, any quantity which may take on
complex values will be denoted by a tilde, for example $\tilde{\varphi}$. As
much as possible, we will endeavor to represent purely imaginary numbers as real
numbers multiplied by the $\im$, a structure which we can handle
programmatically rather then employing unnecessary complex data types and
complex algorithms.

Computation of our matrix elements will require integration both analytic and
numerical. Because numerical integration may require may evaluations of the
integrand, it is important to be clear which quantities may be considered as
\emph{direct integration variables} and \emph{indirect integration variables}
which must be recalculated for each integrand evaluation, as well as quantities
which are \emph{constant over integration} and will only need to be evaluated at
the beginning or end of the integration computation. For this analysis, we are
doing real space integrals over an approximately spherical system, so our direct
integration variables will be real space spherical coordinates
$\left(r,\chi,\phi\right)$. $r$ and $\phi$ are defined in the conventional way,
while $\chi = \cos\left(\theta\right) = z/r$. Indirect integration variables are
quantities which are functions of the direct integration variables and so must
be reevaluated with each iteration of the numerical integration algorithm.
Indirect integration variables are denoted with Latin script characters in a
calligraphic front, or with Greek letters in a conventional font. We denote
which direct integration variables affect a given indirect integration variable
as subscripts on the indirect variable written within curly brackets. Quantities
which are constant over integration are denoted by letters in a fraktur font.
This convention does not extend to the reserved symbols $k$, $l$, $m$, $\xi$,
$\eta$, $\lambda$, $q$,$W$ and $\kappa$. These symbols retain their meaning as
basis indices and parameters, introduced in the previous sections, and as such
would be constant over integration. As well, the symbol $\delta$ is reserved for
the Kronecker delta symbol or the Dirac delta distribution. We hope that,
despite not being in a fraktur font, these symbols' meanings in context make
clear that these quantities are fixed over integration.

In this Appendix, we will prepare to calculate the form of matrix elements
possible within our software. We begin enumerating the forms of our matrix
elements will go as follow. First, the overlap matrix goes as
\begin{equation}
...
\tag{\ref{equation:overlap-general}}
.
\end{equation}
The Hamiltonian matrix elements $ \mathbb{H} = \bm{\mathbb{T}} + \bm{\mathbb{U}}
$ go as
\begin{align}
...
\tag{\ref{equation:kinetic-general}}
,\\
\left(\bm{\mathbb{U}}\right)_{ij}
    & \equiv
    \int \diff\bm{r}\,
    \sum_{pq}\alpha_{p}^{*}\alpha_{q}
    \mathrm{e}^{\im\left(\bm{k}_{q}-\im\bm{k}_{p}\right)\cdot\bm{r}}
    U_{\bm{r}}
    \left\{ \Phi_{i\bm{r}}^{(p)}u_{\bm{k}_{p}\bm{r}}^{} \right\}^{*}
    \Phi_{j\bm{r}}^{\left(q\right)}u_{\bm{k}_{q}\bm{r}}^{}
\tag{\ref{equation:potential-general}}
.
\end{align}
Finally the Hamiltonian-squared matrix elements $ \mathbb{HH} = \bm{\mathbb{TT}}
+ \bm{\mathbb{UU}} + \bm{\mathbb{TU}} + \bm{\mathbb{UT}} $ go as
\begin{align}
...
\tag{\ref{equation:kinkin-general}}
,\\
\left(\bm{\mathbb{UU}}\right)_{ij}
    & \equiv
    \int \diff\bm{r}\,
    \sum_{pq}\alpha_{p}^{*}\alpha_{q}
    \left\{\mathrm{e}^{\im\bm{k}_{p}\cdot\bm{r}}u_{\bm{k}_{p}\bm{r}}\right\}^{*}
    \mathrm{e}^{\im\bm{k}_{q}\cdot\bm{r}}u_{\bm{k}_{q}\bm{r}}
    U_{\bm{r}}^{2}
    \left\{ \Phi_{i\bm{r}}^{(p)}u_{\bm{k}_{p}\bm{r}}^{} \right\}^{*}
    \Phi_{j\bm{r}}^{\left(q\right)}u_{\bm{k}_{q}\bm{r}}^{}
\tag{\ref{equation:potpot-general}}
,\\
\left(\bm{\mathbb{UT}}\right)_{ij}
    ...
    \Phi_{i\bm{r}}^{(p)*}
    \hat{T}^{(q)}\left\{ \Phi_{j\bm{r}}^{\left(q\right)} \right\}
\tag{\ref{equation:potkin-general}}
,\\
\left(\bm{\mathbb{TU}}\right)_{ij}
    ...
    \Phi_{j\bm{r}}^{\left(q\right)}
\tag{\ref{equation:kinpot-general}}
.
\end{align}
...
We also have the possibility of applying an effective kinetic operator to the
envelopes of either of the states forming our integrand, and as such we define
the following notational nicety.
\begin{equation}
\phi_{klm\zeta;\eta\lambda\left\{\bm{r}\right\}}^{(Wq)(\mathfrak{u})}
    \coloneqq
        \left\{
            \left(1-\mathfrak{u}\right) + \mathfrak{u}\hat{T}^{\left(q\right)}
        \right\}
        \phi_{klm\zeta;\eta\lambda\left\{\bm{r}\right\}}^{(Wq)}
\end{equation}
Above $\mathfrak{u} = \pm 1$ is a "switch" allowing us to selectively turn on or
off the effect of the effective valley kinetic operator in the formulae below.
The form of the above expression may be written as 
\begin{equation}
\phi_{klm\zeta;\eta\lambda\left\{\bm{r}\right\}}^{(Wq)(\mathfrak{u})}
    =
        \sum_{s=-\mathfrak{u}}^{+\mathfrak{u}}
        \mathfrak{E}^{\mathfrak{u}}
        \left(\frac{\mathfrak{r}}{\eta}\right)^{2\mathfrak{u}}
        \mathfrak{H}_{ml;\lambda,s}^{\left(\mathfrak{u}\right)}
        \left(F^{\prime}_{s}\right)
...
,
\end{equation}
where the "radial" part of the expression can be given as
\begin{align}
\left(F_{s}^{\prime}\right)
_{lk;\eta\lambda\left\{r\chi_{q}\right\}}
^{\left(W\mathfrak{u}\right)}
    &\coloneqq
        \left(\frac{\eta}{\vartheta}\right)^{-\frac{3}{2}}
        \sqrt{W}
        \rho^{ \frac{l}{W}-\mathfrak{u}\delta_{W1}-\delta_{s\bar{1}} }
        \e^{-\frac{\rho}{2}}
...
,\\
\vartheta &\coloneqq \sqrt{1 + \left(\lambda^{2}-1\right)\chi_{q}^{2}}
,\\
\rho &\coloneqq \left( r\frac{\vartheta}{\eta} \right)^{W}
\label{definition:rho}
,\\
\mathfrak{l} &= \frac{2l+3-W}{W}
.
\end{align}

We have introduced the following coefficients that go as
\begin{align}
\mathfrak{H}_{ml;\lambda,s}^{\left(\mathfrak{u}\right)}
    & =
        \delta_{s0}
            -
        \mathfrak{u}\left(1-\gamma\lambda^{2}\right)
        \mathfrak{Y}_{ml;s}
,\\
h^{\left(\mathfrak{u}\right)}_{kl,st}
    & =
        \left(1-\mathfrak{u}\right)\delta_{s0}\delta_{t0}
            +
        \mathfrak{u}g_{kl,st}
.
\end{align}
More information on the derivation of the above expression, as well as the
specific values of the $\mathfrak{Y}$ and $g$ constants introduced above are
cumbersome to reproduce here. Their full definition, formulae, and derivation
can be found in the
...
We described the possible symmetrized wavefunctions we wish to consider in
section \ref{equation:wavefunction:multi-valley}. The most general form of
wavefunction we found may be expressed as
\begin{equation}
\tilde{\Psi}_{\varrho,mlk\xi}^{\left(\kappa,i\right)}
    =
        \alpha_{z}^{\left(\kappa,i\right)}
        \tilde{\psi}_{\varrho,mlk\xi}^{\left(\kappa,z\right)}
            +
        \alpha_{x}^{\left(\kappa,i\right)}
        \tilde{\psi}_{\varrho,mlk\xi}^{\left(\kappa,x\right)}
            +
        \alpha_{y}^{\left(\kappa,i\right)}
        \tilde{\psi}_{\varrho,mlk\xi}^{\left(\kappa,y\right)}
\end{equation}
The above valley-indexed single-axis wavefunctions $\tilde{\psi}$ are linked by
the cyclic indexing transformations defined in \eqref{idx:z}, \eqref{idx:x}, and
\eqref{idx:y}.

However, the full wavefunction is not used in all matrix elements. Overlap and
purely kinetic elements do not include the periodic part of the Bloch functions.
Taking all of the above into account our overlap, Hamiltonian, and
Hamiltonian-squared matrix elements are computed with the following formula
\begin{align}
\left(\mathcal{M}\right)_{01}
    &=
        \sum_{\alpha_{q_{0}}^{\left(\kappa_{0},i_{0}\right)}}
        \sum_{\alpha_{q_{1}}^{\left(\kappa_{1},i_{1}\right)}}
        \int_{-\pi}^{+\pi}\diff\phi
        \int_{-1}^{+1}\diff\chi
        ...
        \left(
            \phi
            _{\varrho,mlk\zeta;\eta\lambda\left\{\bm{r}\right\}}
            ^{(Wq)(\mathfrak{u})}
            \Theta^{\left(q\right)}
        \right)_{0}^{*}
...
,\\
    &=
        ...
        \left[
            ...
            \Theta^{\left(q\right)}
        \right]_{01}
        \left[\Theta^{\left(q\right)}\right]_{01}
        \tilde{\mathcal{U}}_{\left\{\bm{r}\right\}}
.
\end{align}
where we may define a \emph{generalized axial Bloch function} that goes as
\begin{equation}
\Theta^{\left(q\right)}
    =
        \e^{\im\bm{k}_{q}\cdot\bm{r}}
        \left[
            \left(1-\mathfrak{s}\right)
            +
            \mathfrak{s}
            \eta^{\left(q\right)}_{\xi}
        \right]
\end{equation}
Much like $\mathfrak{u}$, we have $\mathfrak{s} = 0,1,2$ as a "switch" that
communicates information about the potential operator being used in the matrix
elements. $0$ is for the overlap $\mathbb{S}$ and purely kinetic elements
$\mathbb{K}$ and $\mathbb{KK}$. $\mathfrak{s} = 1$ is for the $\mathbb{U}$,
$\mathbb{TU}$, and the $\mathbb{UT}$ elements. Finally $2$ denotes the
$\mathbb{UU}$ element.
